...
/**
 * The main robot class. Contains the various components of the Robot
 */
public class Robot extends IterativeRobot {
	...
	/**
	 * This autonomous method drives the robot straight with a given velocity >0
	 * = forward, <0 = backward, 0 = stop
	 * 
	 * @param velocity
	 * @param time
	 *            the amount of seconds to drive straight
	 */
	...
	public void debug() {
		...
		} else if (controller.getLeftTrigger() > 0) { // Move left lifter motor
														// down
			shooter.moveLeftLifter(-1);
		} else {
		...
		} else if (controller.getRightTrigger() > 0) { // Move right lifter
														// motor down
			shooter.moveRightLifter(-1);
		} else {
	...
	/**
	 * Handles the driving of the robot. Takes into account if turning, uses the
	 * gyro when going straight, and projects the x and y values of the
	 * controller onto an x^2 graph to make it exponential.
	 */
	private void drive() {
	...
	private void pollInput() {
		if (controller.getButtonA()) { // secondary input
			...
			} else if (controller.getRightTrigger() > 0) { // Move right claw
															// down
				claw.moveRightClaw(-1);
			} else {
	...
	/**
	 * Shifts the robot drive into high gear. Faster but less torque
	 */
	...
	/**
	 * Shifts the robot drive into low gear. Slower but more torque
	 */
	...
	public void teleopPeriodic() {
		...
		/*
		 * if(debugSwitch.get()){ debugMode = true; } else{ debugMode = false; }
		 */
		...
		NIVision.imaqDrawLineOnImage(frame, frame, NIVision.DrawMode.DRAW_VALUE, new NIVision.Point(0, 500),
				new NIVision.Point(640, 410), 0.0f);
		NIVision.imaqDrawLineOnImage(frame, frame, NIVision.DrawMode.DRAW_VALUE, new NIVision.Point(295, 0),
				new NIVision.Point(370, 480), 0.0f);
